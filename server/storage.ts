  ...
  async initializeData(): Promise<void> {
    ...
    for (const tribe of TRIBES_DATA) {
      await this.createTribe({
        ...
        color: tribe.color,
        icon: tribe.icon,
        strengths: Array.isArray(tribe.strengths) ? tribe.strengths.join(',') : tribe.strengths
      });
    }
    ...
    for (const region of MAP_REGIONS) {
      const tribeId = this.getTribeIdByName(region.tribe);

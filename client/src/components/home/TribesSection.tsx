...
const TribesSection = () => {
  ...
  const getTribeColor = (id: string) => {
    const tribe = tribes.find(t => t._id === id);
    if (tribe) {
      return tribe.color?.replace('var(--', '').replace(')', '') || 'nomade';
    }
    ...
    >
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <motion.div 
          ...
          animate={controls}
        >
          {loading && <div className="text-white">Chargement des tribus...</div>}
          {error && <div className="text-red-500">Erreur: {error}</div>}
          {!loading && !error && tribes.map((tribe, index) => (
            <TribeBanner 
              key={tribe._id}
              name={tribe.name}
              ...
              icon={tribe.icon}
              index={index}
              onHover={() => setActiveTribe(tribe._id)}
            ...
            {['Territoire', 'Philosophie', 'Technologies', 'Relation aux autres'].map((attribute, i) => (
              ...
              >
                ...
                <div className="p-4 space-y-3">
                  {tribes.map(tribe => (
                    <div key={`${attribute}-${tribe._id}`} className="flex items-center">
                      <div 
                        className="w-3 h-3 rounded-full mr-2"
                        style={{ backgroundColor: tribe.color?.includes('var') ? 
                          `var(--${getTribeColor(tribe._id)})` : tribe.color }}
                      ></div>
            ...
            <table className="min-w-full divide-y divide-gray-700">
                ...
                <tr>
                  ...
                  {tribes.map(tribe => (
                    <th 
                      ...
                      style={{ 
                        backgroundColor: activeTribe === tribe._id ? 
                          `var(--${getTribeColor(tribe._id)})` : 
                          `var(--${getTribeColor(tribe._id)}80)`
                      }}
                    >
                      {tribe.name}
                    </th>
              ...
              <tbody className="bg-black/40 divide-y divide-gray-700">
                ...
                <tr className="group">
                  ...
                  {tribes.map((tribe, i) => (
                    <td key={tribe._id} className="px-4 py-4 text-sm text-gray-200 text-center group-hover:bg-nomades/10 transition-colors duration-300">
                      {Array.isArray(tribe.strengths) ? tribe.strengths.join(', ') : ''}
                    </td>
                  ))}
                </tr>
